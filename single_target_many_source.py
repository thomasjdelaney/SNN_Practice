...
if not args.debug:
    ...
    # extract the data
    target_cell_v = target_cell.get_data('v').segments[0].analogsignals[0]
    target_spikes = target_cell.get_data('spikes').segments[0].spiketrains if args.record_target_spikes else None
    target_cell_gsyn_exc = target_cell.get_data('gsyn_exc').segments[0].analogsignals[0]
    target_cell_gsyn_inh = target_cell.get_data('gsyn_inh').segments[0].analogsignals[0]
    ...
    # do some plotting
    if args.make_plots:
        plotTargetVWithInhibExcitSpikes(target_cell_v, inhib_source_spiketrains, excit_source_spiketrains)
        plotInhExcSynapticStrengths(target_cell_gsyn_exc, target_cell_gsyn_inh, args.duration)
